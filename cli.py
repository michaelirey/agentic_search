...
def build_ignore_specs(folder: Path) -> list[tuple[PathSpec, Path]]:
    ...
    if repo_root:
        ...
        if gitignore_lines:
            specs.append(
                (PathSpec.from_lines("gitwildmatch", gitignore_lines), repo_root)
            )
        ...
        if root_ignore_lines:
            specs.append(
                (PathSpec.from_lines("gitwildmatch", root_ignore_lines), repo_root)
            )
...
def main():
    ...
    init_parser.add_argument(
        ...
        help=(
            ...
            f"(default: {DEFAULT_INDEX_TIMEOUT_SECONDS})"
        ),
    )
